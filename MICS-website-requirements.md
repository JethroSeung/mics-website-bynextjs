...
>
> **v1.1 变更摘要**：按导师最新指示，①取消后端、后台与在线报名（全站纯静态展示）；②研究方向由 3 个改为 2 个（自然灾害场景下的痛感计算 / 医工交叉-多模态感知），详情内容待定；③招新模块改为纯展示任务、任务待发布；④新增"周末讨论决策清单"（§13）。
...
| `/research/pain` | `/en/research/pain` | 自然灾害场景下的痛感计算 | research.ts | **内容待发布**（结构 + 占位） |
...
3. **研究方向 3→2**：自然灾害场景下的痛感计算（负责人杨承轩）、医工交叉-多模态感知（负责人张栩闻）——详情内容周末与导师讨论（2026-09-04）
